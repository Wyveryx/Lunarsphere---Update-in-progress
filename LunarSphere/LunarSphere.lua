...
function LunarSphere_OnEvent(self, event)
	...
	if (event == "PLAYER_LOGIN") then
		...
		if (LunarSphereSettings.buttonSkin > Lunar.includedButtons) then
			artIndex, artType, width, height, artFilename = Lunar.API:GetArtByCatagoryIndex("button",
				LunarSphereSettings.buttonSkin - Lunar.includedButtons);
			if (artFilename) then
		...
		_G["LSmain"]:SetPoint(LunarSphereSettings.relativePoint, UIParent, LunarSphereSettings.relativePoint,
			LunarSphereSettings.xOfs, LunarSphereSettings.yOfs);
...
function LunarSphere_VariablesLoaded()
	...
	if (LunarSphereSettings.randomSphereTexture == true) then
		Lunar.Sphere:SetSphereTexture(math.random(1,
			(Lunar.includedSpheres + LUNAR_EXTRA_SPHERE_ICON_COUNT + Lunar.API:GetArtCount("sphere"))))
	end
...
--]]
	...
	if (LunarSphereSettings.versionID < 0.402) then
		...
		if (repairedMenu > 0) then
			Lunar.API:Print(LUNARSPHERE_CHAT ..
			repairedMenu .. " broken menu button(s) found. Button database repaired =)");
		end
	end
				...
				if not (LunarSphereSettings.buttonData[index].empty) then
					...
					if (LunarSphereSettings.buttonData[index].actionData) then
						while (string.find(LunarSphereSettings.buttonData[index].actionData, "||||")) do
							LunarSphereSettings.buttonData[index].actionData = string.gsub(
							LunarSphereSettings.buttonData[index].actionData, "||||", "|| ||");
						end
					end
					if (LunarSphereSettings.buttonData[index].actionTexture) then
						while (string.find(LunarSphereSettings.buttonData[index].actionTexture, "||||")) do
							LunarSphereSettings.buttonData[index].actionTexture = string.gsub(
							LunarSphereSettings.buttonData[index].actionTexture, "||||", "|| ||");
						end
					end
				end
			...
			for index = 1, 130 do
				...
				if (LunarSphereSettings.buttonData[index].buttonTypeData) then
					if (string.len(LunarSphereSettings.buttonData[index].buttonTypeData) > (9 * 12)) then
						LunarSphereSettings.buttonData[index].buttonTypeData = string.sub(
						LunarSphereSettings.buttonData[index].buttonTypeData, 1, 9 * 12);
						LunarSphereSettings.buttonData[index].buttonTypeData = LunarSphereSettings.buttonData[index]
						.buttonTypeData .. "000000000";
					end
					if (string.len(LunarSphereSettings.buttonData[index].buttonTypeData) < (9 * 13)) then
						LunarSphereSettings.buttonData[index].buttonTypeData = LunarSphereSettings.buttonData[index]
						.buttonTypeData ..
						string.rep("0", 9 * 13 - string.len(LunarSphereSettings.buttonData[index].buttonTypeData));
					end
				end
			end
		end
			...
			if (LunarSphereSettings.buttonData[index]) then
				if (LunarSphereSettings.buttonData[index].buttonSettings) then
					...
					for tempIndex = 1, 13 do
						newIndex = newIndex ..
						string.sub(LunarSphereSettings.buttonData[index].buttonSettings, (tempIndex - 1) * 3 + 1,
							tempIndex * 3) .. "0000";
					end
	...
	if (LunarSphereSettings.versionID < 0.512) then
		...
		LunarSphereSettings.keepNonEquip = (LunarSphereSettings.keepNonEquip == 1) or
		(LunarSphereSettings.keepNonEquip == true);
		LunarSphereSettings.keepWeapons = (LunarSphereSettings.keepWeapons == 1) or
		(LunarSphereSettings.keepWeapons == true);
	...
	if (LunarSphereSettings.versionID < 0.515) then
		...
		Lunar.API:Print(LUNARSPHERE_CHAT ..
		"Due to some issues with some armor and weapons in previous LunarSphere versions, it is adviced that if you have weapons or armor assigned to any LunarSphere button that you re-assign that item in order to update its item link data. I wanted to do an automatic database conversion for everyone, but it didn't turn out well =( -Moongaze");
	end

	if (LunarSphereSettings.versionID < 0.518) then
		LunarSphereSettings.versionID = 0.518
		Lunar.API:Print(LUNARSPHERE_CHAT ..
		"With version of LunarSphere has new code for handling item assignments. If you have a button that is not using an item properly, try re-assigning the item in this new version. Thanks! -Moongaze");
	end
	...
	if (LunarSphereSettings.versionID < 0.520) then
		...
		for buttonID = 0, 130 do
			if (LunarSphereSettings.buttonData[buttonID]) then
				for clickType = 1, 3 do
					for stance = 0, 12 do
						buttonType, cursorType, objectName, objectTexture = Lunar.Button:GetButtonData(buttonID, stance,
							clickType);
						if (cursorType == "item") and (string.find(objectName, "%a") == nil) and (string.len(objectName) > 0) then
							objectName = "item:" .. objectName;
							Lunar.Button:SetButtonData(buttonID, stance, clickType, buttonType, cursorType, objectName,
								objectTexture);
						end
					end
				end
					...
					for stance = 0, 12 do
						...
						if (buttonType) then
							if (buttonType >= 112) and (buttonType < 130) then
								...
								if (newName) then
									Lunar.Button:SetButtonData(buttonID, stance, clickType, buttonType, cursorType,
										newName, objectTexture);
								end
							end
						end
					end
		...
		if (LunarSphereGlobal.speechLibrary) then
			...
			if (LunarSpeechLibrary) then
				if (table.getn(LunarSphereGlobal.speechLibrary) == table.getn(LunarSpeechLibrary)) then
					Lunar.API:Print(LUNARSPHERE_CHAT ..
					"Speech Library - Your database was converted to a new format. Your settings were kept intact. ^_^ -Moongaze");
					for index = 1, table.getn(LunarSpeechLibrary) do
						LunarSpeechLibrary[index].id = index;
					end
				else
					Lunar.API:Print(LUNARSPHERE_CHAT ..
					"Speech Library - The database was rebuilt. All custom scripts are intact, however, the script settings (spell assignment, odds, chat channels) had to be reset. Sorry =( -Moongaze");
			...
			if (LunarSphereGlobal.speechLibrary) then
				for index = 1, table.getn(LunarSphereGlobal.speechLibrary) do
					...
					if (LunarSphereGlobal.speechLibrary[index].default) then
						...
						local scriptName, _ = string.match(LunarSphereGlobal.speechLibrary[index].default,
							"LIB_NAME_(.*)-(%d+)");
						...
						for key, value in pairs(LunarSphereGlobal.speechLibrary[index]) do
							if type(key) == "number" then
								LunarSpeechLibrary.script[index].speeches[key] = LunarSphereGlobal.speechLibrary[index]
								[key];
					...
					LunarSpeechLibrary.script[index].speeches.speechCount = table.getn(LunarSpeechLibrary.script[index]
					.speeches);
				end
	...
	if (LunarSphereSettings.versionID < 0.809) then
		...
		if (LunarSphereSettings.sphereTextType == 1) or (LunarSphereSettings.sphereTextType == 2) then
			...
			Lunar.API:Print(LUNARSPHERE_CHAT ..
			"It appears that you were using \"Sphere Button Count\" or \"Sphere Button Cooldown\" as your sphere text event.");
			Lunar.API:Print(LUNARSPHERE_CHAT ..
			"These events have been removed and replaced with new custom events that lets you set the count or cooldown to be displayed.");
			Lunar.API:Print(LUNARSPHERE_CHAT ..
			"Please cheack the LunarSphere Settings window, in the Sphere tab, for these new options. Thanks! -Moongaze");
		end
	end
	...
	if (LunarSphereSettings.versionID < 1.52) then
		...
		LunarSphereSettings.gaugeColor[LS_EVENT_LUNAR_POWER] = Lunar.Sphere:GetGaugeColorDefaultSettings(
		LS_EVENT_LUNAR_POWER);
		LunarSphereSettings.gaugeColor[LS_EVENT_INSANITY] = Lunar.Sphere:GetGaugeColorDefaultSettings(LS_EVENT_INSANITY);
		LunarSphereSettings.gaugeColor[LS_EVENT_MAELSTROM] = Lunar.Sphere:GetGaugeColorDefaultSettings(
		LS_EVENT_MAELSTROM);
		LunarSphereSettings.gaugeColor[LS_EVENT_RUNE] = Lunar.Sphere:GetGaugeColorDefaultSettings(LS_EVENT_RUNE);
		LunarSphereSettings.gaugeColor[LS_EVENT_SOUL_SHARD] = Lunar.Sphere:GetGaugeColorDefaultSettings(
		LS_EVENT_SOUL_SHARD);
		LunarSphereSettings.gaugeColor[LS_EVENT_HOLY_POWER] = Lunar.Sphere:GetGaugeColorDefaultSettings(
		LS_EVENT_HOLY_POWER);
		LunarSphereSettings.gaugeColor[LS_EVENT_CHI] = Lunar.Sphere:GetGaugeColorDefaultSettings(LS_EVENT_CHI);
		LunarSphereSettings.gaugeColor[LS_EVENT_ARCANE_CHARGE] = Lunar.Sphere:GetGaugeColorDefaultSettings(
		LS_EVENT_ARCANE_CHARGE);
		...
		if (playerLoginCheck[1] == 0.611) then
			...
			for buttonID = 0, 130 do
				if (LunarSphereSettings.buttonData[buttonID]) then
					for clickType = 1, 3 do
						for stance = 0, 12 do
							buttonType, cursorType, objectName, objectTexture = Lunar.Button:GetButtonData(buttonID,
								stance, clickType);
				...
				for scriptIndex = 1, table.getn(LunarSpeechLibrary.script) do
					if LunarSpeechLibrary.script[scriptIndex].spells then
						for spellIndex = 1, table.getn(LunarSpeechLibrary.script[scriptIndex].spells) do
							LunarSpeechLibrary.script[scriptIndex].spells[spellIndex] = string.gsub(
							LunarSpeechLibrary.script[scriptIndex].spells[spellIndex], tempSummon, "");
						end
					end
				end
			end
...
function LunarSphere_cow()
	...
	if (LunarSphereSettings.gievcowlevel) then
		...
		if not (_G["cowFrame"]) then
			...
			for y = 1, 10 do
				for x = 1, 16 do
					subFrame = Lunar.API:CreateFrame("Button", "cowSubFrame" .. x + ((y - 1) * 16), cowFrame, width,
						height, "Interface\\DialogFrame\\UI-DialogBox-Background", nil, 0);
